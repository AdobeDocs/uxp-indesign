...
| --- | --- | --- |
| date | `Date` \| `string` | `[Optional]`                  `Date` : A JavaScript Date object. The time zone is set to the local operation-system time-zone value.                         Time can have up to nanosecond resolution; however, when converting to or from a JavaScript Date value,                         time resolution is reduced to milliseconds.\</br\>                  `iso8601Date`: A string containing a date-time in ISO 8601 format; for example: "2007-04-10T17:54:50+01:00" |
...
**Returns**: `number` - 0 if the two values are the same,\<br/\>
                   1 if this date-time is later than the comparison value,\<br/\>
                   -1 if this date-time is earlier than the comparison value.\<br/\>`number`  
